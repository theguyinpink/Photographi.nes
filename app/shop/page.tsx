            ...
            {products.map((p) => {
              ...
              return (
                <Link key={p.id} href={`/product/${p.id}`} className="group">
                  <div className="overflow-hidden rounded-[28px] border border-black/10 bg-white transition hover:border-black/20">
                    <div className="p-3">
                      {/* volontairement <img> ici pour éviter les erreurs Next/Image */}
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={src}
                        alt={p.title}
                        className="w-full h-auto object-contain rounded-[20px] bg-black/3"
                        loading="lazy"
                      />
                    </div>
